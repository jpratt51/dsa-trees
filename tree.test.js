...
describe("sumValues", function () {
    it("sums simple trees", function () {
        expect(smallTree.sumTreeNums()).toEqual(3);
    });

    it("sums more complicated trees", function () {
        expect(largeTree.sumTreeNums()).toEqual(36);
    });

    it("sums up an empty tree", function () {
        expect(emptyTree.sumTreeNums()).toEqual(0);
    });
});

describe("countEvens", function () {
    it("counts nodes in simple trees", function () {
        expect(smallTree.countTreeEvens()).toEqual(1);
    });

    it("counts Treenodes in more complicated trees", function () {
        expect(largeTree.countTreeEvens()).toEqual(4);
    });

    it("counts empty", function () {
        expect(emptyTree.countTreeEvens()).toEqual(0);
    });
});

describe("numGreater", function () {
    it("counts nodes in small trees", function () {
        expect(smallTree.treeNumGreater(0)).toEqual(2);
        expect(smallTree.treeNumGreater(1)).toEqual(1);
        expect(smallTree.treeNumGreater(2)).toEqual(0);
        expect(smallTree.treeNumGreater(3)).toEqual(0);
    });

    it("counts nodes in larger trees", function () {
        expect(largeTree.treeNumGreater(0)).toEqual(8);
        expect(largeTree.treeNumGreater(4)).toEqual(4);
        expect(largeTree.treeNumGreater(8)).toEqual(0);
    });

    it("counts nodes in an empty  tree", function () {
        expect(emptyTree.treeNumGreater(0)).toEqual(0);
    });
});
